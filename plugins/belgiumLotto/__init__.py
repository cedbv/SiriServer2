...
class lottoBelgium(Plugin):
    ...
    def lotto(self, speech, language, regex):
        ...
        if typetirage == "loto" or typetirage == "lotto":
            typetirage = "lotto"
            if self.assistant.timeZoneId != "Europe/Brussels":
        ...
        result_say = ""
        try:
            response = urllib2.urlopen("http://www.lotto.be/soap2/{0}.asmx/LatestDraw?User=appsolution&Password=appsolutionwebservice".format(typetirage), timeout=5).read()
